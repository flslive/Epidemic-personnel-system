    ...
    while (True):
        ...
        if flag == 13:
            output_path = os.path.join(
                "D:\\code\\Epidemic_personnel_system\\app01\\face\\getCamera.jpg")
            cv2.imwrite(output_path, img)
            voicePrompt("已拍照, 请按 Esc 键完成更新 !")
            time.sleep(2)
        if flag == 27:
            break
...
def go_api(image, nid):
    userId = models.Register.objects.filter(id=nid).first().name
    print(userId)
    options = {}
    ...
    result = client.updateUser(str(image, 'utf-8'), IMAGE_TYPE, GROUP, userId, options)
    print(result)
    if result['error_msg'] == 'SUCCESS':
        voicePrompt('更新成功')
    if result['error_msg'] == 'liveness check fail':
        voicePrompt("人脸动态检测失败，请按esc键重新识别。")
        time.sleep(3)
        return -1
    else:
        return 0
