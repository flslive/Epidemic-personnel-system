...
# 上传到百度 api 进行人脸注册
def go_api(image):
    userId = models.Register.objects.last().name
    options = {}
    options['user_info'] = '123'
    options['quality_control'] = 'NORMAL'
    options['liveness_control'] = 'LOW'
    options["action_type"] = "REPLACE"
    result = client.addUser(str(image, 'utf-8'), IMAGE_TYPE, GROUP, userId, options)
    print(result)

    if result['error_msg'] == 'SUCCESS':
        voicePrompt('注册成功')
    if result['error_msg'] == 'pic not has face':
        voicePrompt("检测不到人脸,注册失败。")
        time.sleep(3)
        return -1
    else:
        return 0
